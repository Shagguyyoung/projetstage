...
<div x-data="{
    ...
    <div class="px-6 py-4 border-b flex justify-between items-center">
        <h2 class="text-xl font-semibold text-gray-800">Re-Inscription</h2>
    </div>
    ...
    <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            ...
            <template x-for="item in [
                {id:'ISMDA23UC001', password:'123456', centre:'Abidjan', pays:'Côte d\'Ivoire', nom:'Babo', prenom:'Josué', tel:'0707070707', mail:'babo@example.com', statut:'Non reçu', decision:''},
                {id:'ISMDA23UC002', password:'abcdef', centre:'Bouaké', pays:'Côte d\'Ivoire', nom:'Koné', prenom:'Aminata', tel:'0589898989', mail:'kone@example.com', statut:'Reçu', decision:''}
            ]" :key="item.id">
                <tr class="hover:bg-gray-50">

                    <td class="px-6 py-3 text-sm min-w-[120px]" x-text="item.id"></td>
                    <td class="px-6 py-3 text-sm min-w-[120px]" x-text="item.password"></td>
                    <td class="px-6 py-3 text-sm min-w-[120px]" x-text="item.centre"></td>
                    <td class="px-6 py-3 text-sm min-w-[120px]" x-text="item.pays"></td>
                    <td class="px-6 py-3 text-sm min-w-[140px]" x-text="item.nom"></td>
                    <td class="px-6 py-3 text-sm min-w-[140px]" x-text="item.prenom"></td>
                    <td class="px-6 py-3 text-sm min-w-[140px]" x-text="item.tel"></td>
                    <td class="px-6 py-3 text-sm min-w-[180px]" x-text="item.mail"></td>

                    <!-- Statut Badge -->
                    <td class="px-6 py-3 text-sm min-w-[100px]">
                        <span class="px-3 py-1 text-xs rounded-full text-white"
                            :class="item.statut === 'Reçu' ? 'bg-green-600' : 'bg-gray-500'"
                            x-text="item.statut">
                        </span>
                    </td>
                    ...
                    <td class="px-6 py-3 min-w-[100px]">
                        <button @click="item.statut = 'Reçu'"
                            class="bg-blue-600 text-white px-4 py-2 text-sm rounded hover:bg-blue-700 transition-all">
                            Reçu
                        </button>
                    </td>

                    <!-- Décision -->
                    <td class="px-6 py-3 text-sm min-w-[180px] flex flex-col gap-2">
                        <!-- Texte promotion -->
                        <div class="font-medium text-gray-800">Promotion</div>

                        <!-- Listbox / select -->
                        <select x-model="item.decision" class="border-gray-300 rounded px-2 py-1 text-sm">
                            <option value="A">Option A</option>
                            <option value="B">Option B</option>
                            <option value="C">Option C</option>
                        </select>

                        <!-- Bouton Inscrit pour chaque personne -->
                        <button @click="alert(`Inscription effectuée pour ${item.nom} ${item.prenom}`)"
                            class="bg-green-600 text-white px-4 py-1 rounded hover:bg-green-700 text-sm transition-all">
                            Inscrit
                        </button>
                    </td>
